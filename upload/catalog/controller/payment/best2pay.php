...
class ControllerPaymentBest2pay extends Controller {
	public function index() {
        ...
        $signature = base64_encode(md5($register_data['sector'] . $register_data['amount'] . $register_data['currency'] . $this->config->get('best2pay_password')));
		...
		if ($order_id) {
			...
			$this->data['sector'] = $this->config->get('best2pay_sector');
			$this->data['id'] = $order_id;
			$this->data['signature'] = base64_encode(md5($this->config->get('best2pay_sector') . $order_id . $this->config->get('best2pay_password')));
